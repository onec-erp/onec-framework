...
final class Components {
    ...
    /**
     * A compact action button: optional leading icon + label over an {@code onno://}
     ...
     * opens the stored file in a new tab (an {@code onno://open/...} action). Reads like the form's
    ...
    /**
     * The {@code onno://open/...} action that opens a stored file: an absolute {@code http(s)}
    ...
    private static String openActionFor(String url) {
        if (url.startsWith("http://") || url.startsWith("https://")) {
            return "onno://open/" + url;
        }
        return "onno://open/" + (url.startsWith("/") ? url.substring(1) : url);
    }
    ...
    static Map<String, Object> geoFieldRow(String label, String value, String hint, Palette p) {
        ...
        geo.put("value", value);
        geo.put("label", label);
        // Single "geo" prop key matching the onno-geo element's `set geo(...)`, like the
        // widget/form/list bridges (DivKit assigns each custom_props key as an element property).
        Map<String, Object> map = Div.custom("onno-geo", Map.of("geo", geo));
        Div.matchWidth(map);
        Div.height(map, 200);
        Div.corner(map, 10);
