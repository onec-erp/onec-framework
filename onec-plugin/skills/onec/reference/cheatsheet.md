# onec API cheat sheet

Dense lookup for the framework's public surface. Verify against `onec-framework/src/main/java/com/onec`
if something looks off — the code wins, and you should fix this file if it has drifted.

## Annotations (package `com.onec.annotations`)
...
| `secret` | boolean | `false` | Encrypted at rest (needs `onec.security.secret-key`), masked as `__SECRET_SET__` on read. |
...
## Base classes & types (package `com.onec.model`, `com.onec.types`)
...
## Lifecycle & rules (packages `com.onec.lifecycle`, `com.onec.rules`)
...
## Posting (package `com.onec.posting`)
...
## Query engine (package `com.onec.query`)
...
## Repositories (package `com.onec.repository`)
...
- `OutboxWriter.append(aggregateType, aggregateId, eventType, payload)` → `onec_outbox`; relayed by
  `onec-kafka-starter` as CloudEvents.

## Schema & migration (packages `com.onec.schema`, `com.onec.migration`)

- `SchemaMode{APPLY,PLAN,VALIDATE,OFF}` ← `onec.schema.mode`. `SchemaUpgrader.run(jdbi)` diffs
  desired `SchemaModel` vs previous `SchemaSnapshot` (JSON in `onec_schema_history`) vs live DB.
...
## MCP tools (onec-mcp-starter, served at `/mcp`)
...
gated by `onec.mcp.writes-enabled`); `post_document`, `unpost_document` (gated by
`onec.mcp.posting-enabled`). All run as the authenticated user under the same deny-by-default RBAC.
