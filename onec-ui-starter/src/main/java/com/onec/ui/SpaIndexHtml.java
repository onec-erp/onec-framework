 ...
 * {@code window.__onecBasePath} to set its router basename and to build shareable deep links, so the
 * server substitutes {@code onec.ui.path} into the served HTML — replacing the {@code "__ONEC_BASE_PATH__"}
...
class SpaIndexHtml {
    ...
    static final String PLACEHOLDER = "__ONEC_BASE_PATH__";
    ...
    private static Resource inject(String basePath) {
        ...
        try {
            ...
            // Replace the quoted token so the surrounding `window.__onecBasePath = "…"` assignment is
