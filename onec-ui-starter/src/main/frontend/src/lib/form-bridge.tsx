...
/**
 * Bridges DivKit's {@code div-custom} block of type {@code onec-form} to the React
 ...
 * implements {@code onec-form}; this is the web implementation. Mirrors the widget
...
class OnecFormElement extends HTMLElement {
...
export function defineFormElement() {
  ...
  if (!customElements.get("onec-form")) {
    customElements.define("onec-form", OnecFormElement);
  }
  defined = true;
}
defineFormElement();
...
export const FORM_CUSTOM_COMPONENTS = new Map<string, { element: string }>([
  ["onec-form", { element: "onec-form" }],
]);

/** Portals every live {@code <onec-form>} to its React form. Mount once, inside the Router. */
