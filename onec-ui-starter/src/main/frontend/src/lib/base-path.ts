// The SPA can be mounted under a configurable URL prefix (server property `onec.ui.path`, default
// `/ui`). The server bakes the live value into the served index.html as `window.__onecBasePath`,
// replacing the `__ONEC_BASE_PATH__` placeholder, so the client knows its mount prefix synchronously
...
const PLACEHOLDER = "__ONEC_BASE_PATH__";

function resolveBasePath(): string {
  const raw = window.__onecBasePath;
