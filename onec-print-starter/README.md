...
(`onec.print.enabled` defaults to `true`). To turn it off:
...
| Key | Default | Purpose |
|-----|---------|---------|
| `onec.print.enabled` | `true` | Master switch. |
| `onec.print.base-packages` | *application's auto-configuration packages* | Packages scanned for `@PrintTemplate` classes. Defaults to your app's base packages. |
| `onec.print.encoding` | `UTF-8` | Charset used to read templates and encode HTML output. |
...
- **Scan scope.** If your `@PrintTemplate` classes live outside the application's base packages, set
  `onec.print.base-packages` explicitly or they won't be registered.
