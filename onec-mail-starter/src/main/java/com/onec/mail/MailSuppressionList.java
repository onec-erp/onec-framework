 ...
 * addresses that damage sender reputation. Backed by {@code onec_mail_suppression}; addresses are
 * normalised to lower-case.
 */
public class MailSuppressionList {

    private static final String DDL =
            "CREATE TABLE IF NOT EXISTS onec_mail_suppression (\n" +
    ...
    public boolean isSuppressed(String address) {
        ...
        return jdbi.withHandle(h -> h.createQuery(
                        "SELECT 1 FROM onec_mail_suppression WHERE _address = :a")
    ...
    public void suppress(String address, String reason, String detail) {
        ...
        jdbi.useHandle(h -> h.createUpdate(
                        "INSERT INTO onec_mail_suppression (_address, _reason, _detail, _created_at) " +
    ...
    public void remove(String address) {
        ...
        jdbi.useHandle(h -> h.createUpdate("DELETE FROM onec_mail_suppression WHERE _address = :a")
                .bind("a", normalise(address))
                .execute());
    }
    ...
    public Optional<String> reasonFor(String address) {
        ...
        return jdbi.withHandle(h -> h.createQuery(
                        "SELECT _reason FROM onec_mail_suppression WHERE _address = :a")
                .bind("a", normalise(address))
                .mapTo(String.class)
                .findFirst());
    }
