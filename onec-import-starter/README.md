...
Generic CSV import endpoints for onec applications.
...
```kotlin
dependencies {
    implementation("com.onec:onec-import-starter:0.1.0")
}
```

The starter auto-configures when `MetadataRegistry` and `onec-ui-starter` services are present.
...
Returns the headers, up to `onec.import.preview-rows` sample rows, and total row count.
...
| --- | --- | --- |
| `onec.import.enabled` | `true` | Master switch. |
| `onec.import.max-file-bytes` | `5242880` | Max uploaded CSV file size. |
| `onec.import.preview-rows` | `20` | Max sample rows returned by preview. |
| `onec.import.max-rows` | `10000` | Max rows processed by one import request. |
