...
`AutoConfigurationPackages`). Point the scanner elsewhere with `onec.scan-packages` if your entities
...
| Key | Default | Purpose |
|-----|---------|---------|
| `onec.scan-packages` | *(empty — falls back to `@SpringBootApplication` packages)* | Base packages scanned for catalog/document/register/enumeration/constant classes. A non-empty list overrides the auto-detected packages. If neither is resolvable, startup fails with an explanatory `IllegalStateException`. |
